...
int32_t fgetRawADC ()
{
	...
	//TODO: ReWrtie HX711 interface for RMT peripheral
	ets_delay_us(2);
...
int32_t		freadAdc		(stu_mesCell* dataPoint, int* i32_limit)
{
	int32_t err = 0;
	int count = fgetRawADC();
...
void fADCConfig	(uint32_t CMDlet)
{
	...
	case CMD_lmlc:
		...
		{
			pc_configOut = malloc(64);
			unsigned int ui32_limitRaw = 0;
			ui32_limitRaw = strtol(pc_value, NULL, 10);
			unsigned int ui32_limitConverted = 0;
