...
esp_err_t ds18b20_read_scratchpad(onewire_bus_handle_t handle, const uint8_t *rom_number, uint8_t* ui8_THregister, uint8_t* ui8_TLregister)
{
    ESP_RETURN_ON_ERROR(onewire_bus_reset(handle), TAG, "error while resetting bus"); // reset bus and check if the device is present
    ...
    *ui8_THregister = scratchpad.th_user1;
    *ui8_TLregister = scratchpad.tl_user2;
    return ESP_OK;
}


//TODO: update routines
esp_err_t ds18b20_write_scratchpad(onewire_bus_handle_t handle, const uint8_t *rom_number, uint8_t* ui8_THregister, uint8_t* ui8_TLregister)
{
    ESP_RETURN_ON_ERROR(onewire_bus_reset(handle), TAG, "error while resetting bus"); // reset bus and check if the device is present
